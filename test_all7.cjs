...
async function test() {
  const { data, error } = await supabase.from('ods_checklist').select('*').limit(0);
  console.log('Error:', error);
  console.log('Data:', data);
  // fetch columns using postgrest directly doesn't easily show columns if empty. Let's do an error triggering
  const { error: e2 } = await supabase.from('ods_checklist').select('non_existent_column');
  console.log('Columns error:', e2);
}
test();
