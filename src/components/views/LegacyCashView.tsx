  ...
  return (
    <div className="p-6 h-full flex flex-col">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-display font-bold text-[var(--color-text-primary)] tracking-wider flex items-center gap-3">
            HISTÓRICO LEGACY
            <span className="bg-red-100 text-red-700 text-xs px-2 py-1 rounded border border-red-200 uppercase tracking-widest">
              Solo Lectura
            </span>
          </h1>
          <p className="text-[var(--color-text-muted)] mt-1 font-mono text-sm">
            Registro histórico de operaciones (v1). Esta información está congelada.
          </p>
        </div>
      </div>

      <div className="flex-1 overflow-auto rounded-xl border border-[var(--color-border-primary)] bg-[var(--color-bg-surface)]">
        <table className="w-full text-sm text-left text-[var(--color-text-secondary)]">
          <thead className="text-xs text-[var(--color-text-primary)] uppercase bg-[var(--color-bg-primary)] border-b border-[var(--color-border-primary)] font-display tracking-wider sticky top-0">
            <tr>
          ...
          <tbody className="divide-y divide-slate-200 font-mono">
            {transactions.map((tx) => (
              <tr key={tx.id} className="hover:bg-[var(--color-bg-primary)] transition-colors">
                <td className="px-4 py-3">{tx.date}</td>
                <td className="px-4 py-3 text-[#7A1B28]">{tx.orderNumber || tx.referenceNumber || '-'}</td>
                <td className="px-4 py-3 text-[var(--color-text-primary)]">{tx.customerName}</td>
            ...
            {transactions.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-[var(--color-text-muted)] italic">
