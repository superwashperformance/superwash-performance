    ...
    <div className="flex flex-col gap-6 p-4 md:p-8 max-w-7xl mx-auto w-full relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Base de datos centralizada para crear, buscar y gestionar clientes.</p>
        </div>
      ...
      {/* Search Bar */}
      <div className="flex items-center gap-3 bg-slate-900 border border-white/10 rounded-xl px-4 py-2.5 w-full max-w-md shadow-inner">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredCustomers.length === 0 ? (
          <div className="col-span-full nike-card p-12 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="font-display text-xl text-white mb-1">No se encontraron clientes</h3>
            <p className="text-xs text-slate-400 mb-4">Intenta con otra búsqueda o crea un nuevo cliente.</p>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] text-slate-400 font-mono">Desde {cust.createdAt}</span>
                      <button
                        onClick={() => openEditCustomerModal(cust)}
                        className="p-1.5 bg-slate-800 hover:bg-[#00E5FF] text-slate-300 hover:text-black rounded-lg transition-colors"
                  ...
                  <h3 className="font-display text-2xl text-white tracking-wide">{cust.fullName}</h3>

                  <div className="flex flex-col gap-1.5 mt-3 text-xs text-slate-300 font-mono">
                    <div className="flex items-center gap-2">
                ...
                <div className="border-t border-white/10 pt-3">
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <button
                      ...
                      className="text-[10px] font-mono font-bold text-[#00E5FF] hover:underline flex items-center gap-1"
                    >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
          <div className="nike-card p-6 w-full max-w-md flex flex-col gap-4 border-white/20 shadow-2xl animate-scale-up">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-3">
              <div>
                <label className="text-xs text-slate-400 mb-1 block">Nombre Completo / Razón Social *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full bg-slate-900 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-[#00E5FF] outline-none font-medium"
                />
              </div>

              <div>
                <label className="text-xs text-slate-400 mb-1 block">Cédula / RIF / DNI *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-slate-400 mb-1 block">Teléfono Principal *</label>
                  <input
                    type="text"
                    required
              ...
              <div>
                <label className="text-xs text-slate-400 mb-1 block">Dirección de Domicilio / Fiscal</label>
                <input
                  type="text"
              ...
              <div className="mt-2 pt-3 border-t border-white/10 flex flex-col gap-3">
                <span className="font-mono text-xs font-bold text-[#00E5FF] flex items-center gap-1.5 uppercase">
                  <Car className="w-4 h-4 text-[#00E5FF]" /> Registrar Vehículo del Cliente (Opcional)
                </span>

                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-[10px] text-slate-400 block mb-1">Placa / Matrícula</label>
                    <input
                      type="text"
                      placeholder="Ej. AA991GT"
                      value={initialVehPlate}
                      onChange={(e) => setInitialVehPlate(e.target.value)}
                      className="w-full bg-black border border-white/10 rounded px-2 py-1.5 text-xs text-white font-mono uppercase font-bold focus:border-[#00E5FF] outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-black border border-white/10 rounded px-2 py-1.5 text-xs text-white font-mono focus:border-[#00E5FF] outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-[10px] text-slate-400 block mb-1">Marca</label>
                    <input
                      type="text"
                      placeholder="Ej. Toyota"
                      value={initialVehBrand}
                      onChange={(e) => setInitialVehBrand(e.target.value)}
                      className="w-full bg-black border border-white/10 rounded px-2 py-1.5 text-xs text-white focus:border-[#00E5FF] outline-none"
                    />
                  </div>
                  <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
          <div className="nike-card p-6 w-full max-w-md flex flex-col gap-4 border-white/20 shadow-2xl animate-scale-up">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <div>
                <h3 className="font-display text-xl text-white flex items-center gap-2">
                  <Plus className="w-5 h-5 text-[#00E5FF]" /> ASOCIAR NUEVO VEHÍCULO
                </h3>
                <p className="text-xs text-slate-400">Cliente: <strong className="text-white">{vehicleCustomer.fullName}</strong></p>
              </div>
