...
export const TreasuryView: React.FC = () => {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const [accs, movs, debts, docs, sessions, custs] = await Promise.all([
            ...
            <table className="w-full text-left text-sm text-slate-700">
              <thead className="bg-slate-100 font-display text-sm tracking-wider uppercase text-slate-600 border-b border-slate-200">
                <tr>
                  ...
                  <th className="p-4">MÉTODO / CONCEPTO</th>
        ...
        <div className="flex flex-col gap-4 animate-fade-in">
          <div className="glass-card p-4 border-slate-200 flex flex-col sm:flex-row items-center gap-4">
            ...
            >
              ...
              {customers.map(c => (
                 <option key={c.id} value={c.id}>{c.fullName || 'Sin nombre'} - {c.documentId || ''}</option>
              ))}
            </select>
          </div>
