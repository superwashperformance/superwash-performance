...
export const InventoryView: React.FC<InventoryViewProps> = ({ inventory, inventoryMovements, onUpdateStock, onDeleteProduct }) => {
  ...
  return (
    <div className="flex flex-col gap-6 p-4 md:p-8 max-w-7xl mx-auto w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="font-display text-3xl text-slate-900 tracking-wide flex items-center gap-2">
            INVENTARIOS DUALES SEPARADOS <Package className="w-6 h-6 text-cyan-600" />
          </h2>
      ...
      {/* Main View Toggle */}
      <div className="flex items-center gap-2 bg-black/60 p-1.5 rounded-2xl border border-slate-200 w-fit">
        <button
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-2.5 rounded-xl font-display text-base tracking-wider uppercase transition-all ${
            viewMode === 'kardex'
              ? 'bg-purple-500 text-slate-900 font-bold shadow-lg shadow-purple-500/20'
              : 'text-slate-500 hover:text-slate-900 hover:bg-white/5'
          }`}
        >
        ...
        <>
          ...
          <div className="flex items-center justify-between gap-4">
            <input
              ...
              className="w-full max-w-md bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-900 focus:border-[#00E5FF] outline-none"
            />
                ...
                {filteredItems.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-white/5 transition-colors">
                      ...
                      <td className="p-3 text-slate-700">{item.responsiblePerson}</td>
                      <td className="p-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            onClick={() => onUpdateStock(item.id, Math.max(0, item.stock - 1))}
                            className="w-7 h-7 rounded-lg bg-slate-100 hover:bg-slate-700 text-slate-900 font-bold flex items-center justify-center"
                          ...
                          <button
                            ...
                            className="w-7 h-7 rounded-lg bg-[#00E5FF]/20 hover:bg-[#00E5FF] hover:text-black text-cyan-600 font-bold flex items-center justify-center transition-all"
                      ...
                      <td className="p-3 text-center">
                        {onDeleteProduct && (
                          <button
                            ...
                            className="p-1.5 bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-slate-900 rounded-lg transition-colors border border-red-500/30"
        ...
        <>
          ...
          <div className="nike-card p-4 overflow-x-auto border-purple-500/30 shadow-[0_0_30px_rgba(168,85,247,0.1)]">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-display text-lg text-slate-900 uppercase tracking-wider flex items-center gap-2">
                Historial de Movimientos
              </h3>
              <div className="text-xs text-slate-500 font-mono">Últimos {inventoryMovements.length} movimientos</div>
            </div>
              ...
              <tbody className="divide-y divide-white/5 font-sans">
                {inventoryMovements.map((mov) => (
                  <tr key={mov.id} className="hover:bg-white/5 transition-colors">
                    <td className="p-3 font-mono text-slate-500">{mov.date}</td>
                    <td className="p-3">
                    ...
                    <td className="p-3 font-bold text-slate-900">{mov.itemName}</td>
                    <td className="p-3 font-mono font-bold text-slate-900">{mov.quantity > 0 ? `+${mov.quantity}` : mov.quantity}</td>
                    <td className="p-3 text-slate-700 italic">{mov.reason}</td>
                    <td className="p-3 text-slate-500">{mov.responsiblePerson}</td>
                  </tr>
                ))}
              </tbody>
            </table>
