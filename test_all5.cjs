...
async function test() {
  const { data, error } = await supabase.from('service_orders').select('*, customers ( full_name, phone ), vehicles ( plate, brand, model, year, color ), ods_photos ( id, photo_url, category, caption, created_at ), ods_services ( id, service_name, category, unit_price, quantity, total_price ), ods_checklist ( id, item_key, label, condition, notes ), ods_damage_markers ( id, x_pos, y_pos, view_name, damage_type, severity, description )').limit(1);
